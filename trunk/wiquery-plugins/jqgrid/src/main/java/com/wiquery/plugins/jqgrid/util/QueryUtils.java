...
public class QueryUtils
{
    private static class IOrderComparator<T extends Serializable> implements Comparator<T>
    {
        ...
        public int compare( T o1, T o2 )
        {
            try
            {
                final Object ov1 = ReflectionUtils.getPropertyValue( o1, order.getProperty() );
                final Object ov2 = ReflectionUtils.getPropertyValue( o2, order.getProperty() );
                if( ov1 instanceof Comparable && ov2 instanceof Comparable )
                {
                    final Comparable c1 = (Comparable)ov1;
                    final Comparable c2 = (Comparable)ov2;
    ...
    public static <T extends Serializable> T matchQuery( T bean, T searchBean, String... searchFields )
    {
        ...
        {
            final String propertyName = filter;               
            try
            {
                final Object rValue  = ReflectionUtils.getPropertyValue(searchBean, propertyName);
                if( rValue != null )
                {
                    final Object value = getPropertyValue( bean, propertyName );
                    final String strVale = value.toString();
    ...
    public static <T extends Serializable> List<T> findSubList(List<T> list, T searchBean, SortInfo info, String... searchFields )
    {
        if( list == null )
            return null;
        final List<T> nlist = new ArrayList<T>();
        for( T bean: list )
        {
            final T result = matchQuery(bean, searchBean, searchFields);
            if( result != null )
            {
                nlist.add(bean);
            }
