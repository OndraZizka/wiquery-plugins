...
import org.wiquery.plugin.jquertytools.tooltip.TooltipBehavior.Effect;
...
public class TooltipAjaxBehaviour extends AbstractDefaultAjaxBehavior {
	...
	private static class JQueryToolsAjaxUiEvent extends JQueryToolsUiEvent {
		...
		protected void execute(JsScopeContext scopeContext) {
			scopeContext.append(
					new StringBuffer()
					.append("var url = '").append(behaviour.getCallbackUrl(true))
					.append("&").append(EVENT).append("=").append(event.name())										
	...
	private static class JQueryToolsOnBeforeShowUiAjaxUiEvent extends JQueryToolsOnBeforeShowUiEvent {
		...
		protected void execute(JsScopeContext scopeContext) {
			scopeContext.append(
					new StringBuffer()
					.append("var top = position.top+'';")
					.append("var left = position.left+'';")
					.append("var url = '").append(behaviour.getCallbackUrl(true))
					.append("&").append(EVENT).append("=").append(event.name())
					.append("&").append(TOP).append("=").append("'+top+'"))
					.append("&").append(LEFT).append("=").append("'+left;")					
					// add position info.
					.append("wicketAjaxGet(url, null,null, function() {return true;})").toString();
		}
	...
	protected final void respond(AjaxRequestTarget target) {
		...
		// if the request if from an event we execute it.
		if(!isEmpty(event)) {
